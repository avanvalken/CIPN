...
```{r}
# D14 DRG
counts <- read.table("D14_DRG/count", header = TRUE)
counts <- counts[,-c(2:6)]
counts <- column_to_rownames(counts, var = "Geneid")

annot <- read.table("D14_DRG/annotation.txt", header = TRUE)
...
```{r}
# D14 PAG
counts <- read.table("D14_PAG/count", header = TRUE)
counts <- counts[,-c(2:6)]
counts <- column_to_rownames(counts, var = "Geneid")

annot <- read.table("D14_PAG/annotation.txt", header = TRUE)
...
```{r}
# D14 spinal_chord
counts <- read.table("D14_spinal_chord/count", header = TRUE)
counts <- counts[,-c(2:6)]
counts <- column_to_rownames(counts, var = "Geneid")

annot <- read.table("D14_spinal_chord/annotation.txt", header = TRUE)

# change colnames to sample names
colnames(counts) <- annot$sample

spinal_chord_d14 <- SummarizedExperiment(assays = as.matrix(counts), 
                               colData = DataFrame(annot),)
names(assays(spinal_chord_d14)) <- "counts"

saveRDS(spinal_chord_d14, "spinal_chord_d14.RDS")

# Process SE
indata <- spinal_chord_d14
indata <- indata[apply(assay(indata,"counts") != 0, 1, mean)>.2,] 
indata  = mkAssay(indata, log = TRUE, counts_to_CPM = TRUE)

spinal_chord_d14 <- indata
...
```{r}
# D28 DRG
counts <- read.table("D28_DRG/count", header = TRUE)
counts <- counts[,-c(2:6)]
counts <- column_to_rownames(counts, var = "Geneid")

annot <- read.table("D28_DRG/annotation.txt", header = TRUE)
...
```{r}
# D28 PAG
counts <- read.table("D28_PAG/count", header = TRUE)
counts <- counts[,-c(2:6)]
counts <- column_to_rownames(counts, var = "Geneid")

annot <- read.table("D28_PAG/annotation.txt", header = TRUE)
...
```{r}
# D28 spinal_chord
counts <- read.table("D28_spinal_chord/count", header = TRUE)
counts <- counts[,-c(2:6)]
counts <- column_to_rownames(counts, var = "Geneid")

annot <- read.table("D28_spinal_chord/annotation.txt", header = TRUE)

# change colnames to sample names
colnames(counts) <- annot$sample[-c(17, 21, 27)] #P1, P5, R3 missing
annot <- annot[-c(17,21,27),]

spinal_chord_d28 <- SummarizedExperiment(assays = as.matrix(counts), 
                               colData = DataFrame(annot),)
names(assays(spinal_chord_d28)) <- "counts"

saveRDS(spinal_chord_d28, "spinal_chord_d28.RDS")

# Process SE
indata <- spinal_chord_d28
indata <- indata[apply(assay(indata,"counts") != 0, 1, mean)>.2,] 
indata  = mkAssay(indata, log = TRUE, counts_to_CPM = TRUE)

spinal_chord_d28 <- indata
```
...
They have what overlaps in the genotypes controlling for treatment
And they have what is unique (controlling for genotype)

that was for genes. THey also have some pathways. 

Me: 

For each time point and for each tissue, take the diffex genes intersect for both genotypes as well, and sex

Between treatments
-Dim reduction
-Diffex between treatments PAC vs VEH
-IPA of pathways

Between genotypes? 
-dim reduction
-Diffex with genotypes 
-IPA of pathways
